...
def revlearn(outfile,flag,*args):
	...
	if flag==0:
		...
		minbetween=args[1]
		maxbetween=args[2]
		trialvec = makeswitches(numswitches,minbetween,maxbetween,np.random.randint(1,9999))
	...
	# sound samples
	rightsnd.play()
	display.text_keypress(testWin,  'You will hear this for correct responses. \n (Press any key to continue)')
	wrongsnd.play()
	display.text_keypress(testWin,  'And this for incorrect responses. \n (Press any key to continue)')
	display.text_keypress(testWin,  'Please make a choice as soon as the "+" appears\n(Press any key when ready)')
	...
	for isTrue in trialvec:
		...
		### save data for each trial (i.e. timestamps, choice, correctness, task and parameters)
