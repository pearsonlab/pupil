...
def pupil_extract(datafile_input_path,
    ...
    extracted_dvs['baseline_pupil_diameter'] = [D_zero_trial0, D_zero_trial1, D_zero_trial2, D_zero_trial3]
    extracted_dvs_avg['baseline_pupil_diameter_avg'] = [np.nanmean(
        [D_zero_trial0, D_zero_trial1, D_zero_trial2, D_zero_trial3])]
    ...
    D_m_trial0 = D_m_range['trial0_spline'].min()
    D_m_trial1 = D_m_range['trial1_spline'].min()
    D_m_trial2 = D_m_range['trial2_spline'].min()
    D_m_trial3 = D_m_range['trial3_spline'].min()

    extracted_dvs['maximal_pupillary_constriction'] = [D_m_trial0, D_m_trial1, D_m_trial2, D_m_trial3]
    extracted_dvs_avg['maximal_pupillary_constriction_avg'] = \
        [np.nanmean([D_m_trial0, D_m_trial1, D_m_trial2, D_m_trial3])]
    extracted_dvs_avg['maximal_pupillary_constriction_std'] = \
        [np.nanstd([D_m_trial0, D_m_trial1, D_m_trial2, D_m_trial3])]

    # <!-- Find the time at which maximum pupillary constriction occurred -->
    local_min_trial0_x_idx = np.argmin(D_m_range['trial0_spline'])
    local_min_trial0_x = D_m_range['time'].iloc[[local_min_trial0_x_idx]].values[0]
    ...
    A_trial0 = D_zero_trial0 - D_m_trial0
    A_trial1 = D_zero_trial1 - D_m_trial1
    A_trial2 = D_zero_trial2 - D_m_trial2
    A_trial3 = D_zero_trial3 - D_m_trial3

    extracted_dvs['constriction_amplitude'] = [A_trial0, A_trial1, A_trial2, A_trial3]
    extracted_dvs_avg['constriction_amplitude_avg'] = \
        [np.nanmean([A_trial0, A_trial1, A_trial2, A_trial3])]
    extracted_dvs_avg['constriction_amplitude_std'] = \
        [np.nanstd([A_trial0, A_trial1, A_trial2, A_trial3])]

    # ---------------------- #
    # DV4: t_L = PLR LATENCY #
    # ---------------------- #
    # <!-- Set time range to be between 0s and the point of maximum pupillary constriction -->
    t_L_range_trial0 = spline_df[(spline_df['time'] >= 0) & (spline_df['time'] <= local_min_trial0_x)]
    ...
    local_min_d2_trial0 = np.argmin(t_L_range_trial0['trial0_spline_2d'])
    t_L_trial0 = t_L_range_trial0['time'].iloc[[local_min_d2_trial0]].values[0]
    ...
    extracted_dvs['plr_latency'] = [t_L_trial0, t_L_trial1, t_L_trial2, t_L_trial3]
    extracted_dvs_avg['plr_latency_avg'] = [np.nanmean([t_L_trial0, t_L_trial1, t_L_trial2, t_L_trial3])]
    extracted_dvs_avg['plr_latency_std'] = [np.nanstd([t_L_trial0, t_L_trial1, t_L_trial2, t_L_trial3])]

    # ---------------------------- #
    # DV5: t_C = CONSTRICTION TIME #
    # ---------------------------- #
    # <!-- Time at which max constriction occurred minus latency time -->
    t_C_trial0 = local_min_trial0_x - t_L_trial0
    t_C_trial1 = local_min_trial1_x - t_L_trial1
    t_C_trial2 = local_min_trial2_x - t_L_trial2
    t_C_trial3 = local_min_trial3_x - t_L_trial3

    extracted_dvs['constriction_time'] = [t_C_trial0, t_C_trial1, t_C_trial2, t_C_trial3]
    extracted_dvs_avg['constriction_time_avg'] = [np.nanmean([t_C_trial0, t_C_trial1, t_C_trial2, t_C_trial3])]
    extracted_dvs_avg['constriction_time_std'] = [np.nanstd([t_C_trial0, t_C_trial1, t_C_trial2, t_C_trial3])]

    # ------------------------ #
    # DV6: t_R = RECOVERY TIME #
    # ------------------------ #
    # <!-- Limit to time range between peak pupil diameter and roughly where it tops out -->
    t_R_range_trial0 = spline_df[(spline_df['time'] >= local_min_trial0_x) & (spline_df['time'] <= 8)]
    ...
    t_R_trial0 = t_R_t2_trial0 - local_min_trial0_x
    ...
    extracted_dvs['recovery_time'] = [t_R_trial0, t_R_trial1, t_R_trial2, t_R_trial3]
    extracted_dvs_avg['recovery_time_avg'] = [np.nanmean([t_R_trial0, t_R_trial1, t_R_trial2, t_R_trial3])]
    extracted_dvs_avg['recovery_time_std'] = [np.nanstd([t_R_trial0, t_R_trial1, t_R_trial2, t_R_trial3])]

    # -------------------------------- #
    # DV7: R_C = RELATIVE CONSTRICTION #
    # -------------------------------- #
    # <!-- (Initial^2 - Maximum Constriction^2) / Initial^2 -->
    R_C_trial0 = (D_zero_trial0**2 - D_m_trial0**2)/D_zero_trial0**2
    ...
    extracted_dvs['relative_constriction'] = [R_C_trial0, R_C_trial1, R_C_trial2, R_C_trial3]
    extracted_dvs_avg['relative_constriction_avg'] = [np.nanmean([R_C_trial0, R_C_trial1, R_C_trial2, R_C_trial3])]
    extracted_dvs_avg['relative_constriction_std'] = [np.nanstd([R_C_trial0, R_C_trial1, R_C_trial2, R_C_trial3])]
...
if __name__ == '__main__':
    ...
    for p_num in good_data:
        ...
        if len(p_num) == 1:
            pupil_extract(
                '../pyanalysis/data/CM000' + p_num + '/lighttest/CM000' + p_num + '_lighttest_norm_pupil.csv',
            ...
            pupil_extract(
                '../pyanalysis/data/CM000' + p_num + '/lighttest/CM000' + p_num + '_lighttest_unnorm_pupil.csv',
            ...
            pupil_extract(
                '../pyanalysis/data/CM00' + p_num + '/lighttest/CM00' + p_num + '_lighttest_norm_pupil.csv',
            ...
            pupil_extract(
                '../pyanalysis/data/CM00' + p_num + '/lighttest/CM00' + p_num + '_lighttest_unnorm_pupil.csv',
