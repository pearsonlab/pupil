...
if strcmp(task,'revlearn') | strcmp(task,'oddball')
    st = [data.soundtime]; %sound ontime
    ts = eyedata.timestamp;
elseif strcmp(task,'darktest') | strcmp(task,'lighttest')
    st = [data.ontime]; %stimulus ontime
    ts = eyedata.timestamp;
end
    ...
    % Find timebins where any sound or stimulus occurred.
